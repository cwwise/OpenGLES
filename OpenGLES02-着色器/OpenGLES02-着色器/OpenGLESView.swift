...
class OpenGLESView: UIView {
    ...
    func setupGLProgram() {
        guard let program = ShaderUtil.loadShader(vertexShaderName: "vert.glsl",
                                                  fragmentShaderName: "frag.glsl") else {
            return
        }
    ...
    func render() {
        ...
        glVertexAttribPointer(GLuint(posSlot), 3, GLenum(GL_FLOAT), GLboolean(GL_FALSE), 0, vertices)
        ...
        glVertexAttribPointer(GLuint(colorSlot), 3, GLenum(GL_FLOAT), GLboolean(GL_FALSE), 0, colors)
